...
ggplot(pA_marginal, aes(x = contrast)) +
  ...
  stat_halfeye(aes(fill = stat(x > 0), slab_alpha = stat(f)), 
               ...
               .width = 0.95,
               size = 1,
               point_interval = "mean_qi", # summarize the distribution with its mean
               show.legend=FALSE) +
  ...
  xlab("Contrast of posterior mean predicted probabilities") + 
...
The mean^[Note that, due to some skewness in the distributions, the mean does not perfectly indicate the values with highest posterior mass.] and 95% interval of the posterior predicted contrast between the treatment and the control group is thus `r round(tidybayes::mean_qi(pA_marginal$contrast)$y, 2)`% [`r round(tidybayes::mean_qi(pA_marginal$contrast)$ymin, 2)`, `r round(tidybayes::mean_qi(pA_marginal$contrast)$ymax, 2)`], or roughly a doubling with the bulk of the posterior mass being above zero. This is a substantial effect given that the posterior predicted mean baseline of taking ACT in the control group is `r round(tidybayes::mean_qi(pA_marginal[pA_marginal$act_any==0,]$.epred)$y, 3)*100`% [`r round(tidybayes::mean_qi(pA_marginal[pA_marginal$act_any==0,]$.epred)$ymin, 3)*100`, `r round(tidybayes::mean_qi(pA_marginal[pA_marginal$act_any==0,]$.epred)$ymax, 3)*100`].

Likewise, Figure \ref{pBplot} illustrates the computed contrast in posterior mean predicted probabilities across subsidy levels. The inference is very similar: there is roughly a doubling of chance of taking an ACT between the control and intervention groups, and the bulk of the posterior masses is above zero. Again, this is a substantial effect. While there is little difference between the three subsidy levels overall, perhaps unsurprisingly, the highest subsidy level (92%) has the highest mean posterior predicted change (`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==40,]$contrast)$y, 2)`% [`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==40,]$contrast)$ymin, 2)`, `r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==40,]$contrast)$ymax, 2)`]), with the effects of the 80% price subsidy (`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==100,]$contrast)$y, 2)`% [`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==100,]$contrast)$ymin, 2)`, `r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==100,]$contrast)$ymax, 2)`]) and the 88% price subsidy (`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==60,]$contrast)$y, 2)`% [`r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==60,]$contrast)$ymin, 2)`, `r round(tidybayes::mean_qi(pB_marginal[pB_marginal$coartemprice==60,]$contrast)$ymax, 2)`]) being almost indistinguishable.

# Conclusion {#conclusion}
In conclusion, this re-analysis finds substantial evidence for the claim of the original study that *'... a very high subsidy ... increases access [to antimalarials]'* (p. 609.). Providing households with high price subsidies roughly doubles the chances of taking an ACT for an illness event with malaria-like symptoms, with a baseline likelihood of around `r round(tidybayes::mean_qi(pA_marginal[pA_marginal$act_any==0,]$.epred)$y, 2)*100`%. This inference largely matches the original authors' findings, namely that '[s]ubsidies of 80 percent or more increase the likelihood that an illness is treated with an ACT by 16-23 percentage points (an 85-118 percent increase) [...]' (p. 628). 
